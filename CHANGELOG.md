...
- **Terminal rendered without its theme/font styling, with a CSP console
  error.** `@xterm/xterm`'s DOM renderer (`Viewport.ts`, `DomRenderer.ts`)
  injects `<style>` elements at runtime with dynamically computed CSS
  (colors, cell dimensions) — content that can't be pinned to a nonce/hash.
  The app's Content Security Policy (`tauri.conf.json`'s `app.security.csp`)
  blocked it because `style-src` only allowed `'self'`. Added
  `'unsafe-inline'` to `style-src` only; `script-src` is unchanged.
