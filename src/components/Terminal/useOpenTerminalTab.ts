 ...
 * registers it in the store under a default "Terminal N" title. Used by
 * BOTH the console header's "New terminal" button (which also switches the
 * panel into Terminal mode) and the in-panel `TerminalTabs` "+" button, so
 * the spawn logic is defined exactly once.
 */
export function useOpenTerminalTab(): () => void {
  ...
  return useCallback((): void => {
    ...
    void terminalBridgeReady()
      ...
      .catch((err) => {
        // The spawn never happened (or the bridge never became ready) — give
        // the reserved number back so it doesn't leak and inflate every
        // subsequent tab's number.
